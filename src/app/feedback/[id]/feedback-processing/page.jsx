...
const ProcessingPage = () => {
    ...
    const fetchFeedback = async () => {
      try {
        const response = await fetch(
          ...
          `http://sos-dev.onedistribution.shop/api/feedback/track/${id}/`
        );
        if (!response.ok) {
  ...
  // Updated getStatusStep to handle new statuses
  ...
  const getStatusColor = (status) => {
    switch (status?.toLowerCase()) {
      case "done":
      case "completed":
        return "#08B69B"; // Green for successful completion
      case "rejected":
        return "#EF4444"; // Red for rejected
      case "closed_without_action":
        return "#6B7280"; // Gray for closed without action
      default:
        return "#99a1af"; // Default gray for in-progress steps
    }
  };
              ...
              >
                {status.toLowerCase() === "rejected" ? (
                  <path
                    ...
                    strokeLinejoin="round"
                  />
                ) : status.toLowerCase() === "closed_without_action" ? (
                  <path
                    d="M12 12H12.01M21 12C21 16.9706 16.9706 21 12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12Z"
                    stroke="#0D394F"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
          ...
          <div className="flex items-center gap-1 mt-5">
            ...
            <div className="flex-1 bg-gray-200 rounded-full h-1 dark:bg-gray-700">
              <div
                className={`h-1 rounded-full ${
                  currentStep >= 4
                    ? `bg-[${getStatusColor(status)}] w-full`
                    : "bg-gray-400 w-0"
                }`}
              ></div>
            </div>
            ...
            >
              {status.toLowerCase() === "rejected" ? (
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M2.7417 15C2.7417 8.26903 8.06067 2.8125 14.622 2.8125C21.1832 2.8125 26.5022 8.26903 26.5022 15C26.5022 21.731 21.1832 27.1875 14.622 27.1875C8.06067 27.1875 2.7417 21.731 2.7417 15ZM10 10L20 20M10 20L20 10"
                  fill={getStatusColor(status)}
                />
              ) : status.toLowerCase() === "closed_without_action" ? (
                <circle cx="15" cy="15" r="12" fill={getStatusColor(status)} />
              ) : (
                <path
                  fillRule="evenodd"
                  clipRule="evenodd"
                  d="M2.7417 15C2.7417 8.26903 8.06067 2.8125 14.622 2.8125C21.1832 2.8125 26.5022 8.26903 26.5022 15C26.5022 21.731 21.1832 27.1875 14.622 27.1875C8.06067 27.1875 2.7417 21.731 2.7417 15ZM19.0211 12.7324C19.3144 12.3111 19.2193 11.7256 18.8086 11.4246C18.3979 11.1237 17.8271 11.2213 17.5338 11.6426L13.5915 17.3045L11.6127 15.2746C11.2558 14.9085 10.6772 14.9085 10.3203 15.274 Blockquote
6C9.9634 15.6407 9.9634 16.2343 10.3203 16.6004L13.0619 19.4129C13.2518 19.6078 13.5157 19.707 13.7835 19.6843C14.0512 19.6616 14.2956 19.5192 14.4517 19.2949L19.0211 12.7324Z"
                  fill={getStatusColor(status)}
                />
              )}
            </svg>
        ...
        {/* Processing Line Block for Mobile */}
            ...
            <div className="flex gap-3">
              <div className="flex flex-col items-center mt-1">
                ...
                >
                  {status.toLowerCase() === "rejected" ? (
                    <>
                      <circle cx="15" cy="15" r="15" fill={getStatusColor(status)} />
                      <path
                        d="M10 10L20 20M10 20L20 10"
                        stroke="white"
                        strokeWidth="2"
                        strokeLinecap="round"
                        strokeLinejoin="round"
